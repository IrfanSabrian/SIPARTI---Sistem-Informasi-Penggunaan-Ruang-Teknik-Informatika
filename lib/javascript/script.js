  ...
  try {
    ...
    document.querySelectorAll(".class").forEach((element) => {
      ...
      const classInfo = schedule.find((info) => {
        ...
        return (
          roomCode === roomId &&
          isTimeInRange(currentTime, info.jam_mulai, info.jam_selesai)
        );
      });
...
async function openRoomModal(roomId) {
  ...
  const streamingLinks = {
    TI11: [
      "https://www.youtube.com/embed/pl_QnwDDYHI?autoplay=1",
      "https://www.youtube.com/embed/UC_S1Oa2ypc?autoplay=1",
      "https://www.youtube.com/embed/EEmgm61cQjE?autoplay=1",
    ],
    TI12: [
      "https://www.youtube.com/embed/lHxr2WQkYdA?autoplay=1",
      "https://www.youtube.com/embed/SPXISvqAQbg?autoplay=1",
      "https://www.youtube.com/embed/nfgnpM28xDA?autoplay=1",
    ],
    TI13: [
      "https://www.youtube.com/embed/pl_QnwDDYHI?autoplay=1",
      "https://www.youtube.com/embed/UC_S1Oa2ypc?autoplay=1",
  ...
  // Cek apakah roomId ada dalam daftar streamingLinks
  const imagesHTML = streamingLinks[roomId]
    ? `<h3>Preview Ruangan</h3>
     ${streamingLinks[roomId]
       .map(
         (link) => `
       <iframe width="100%" height="auto" src="${link}" 
...
function closeModal() {
  ...
  // Menghapus elemen iframe dari modal untuk menghentikan video
  const modalContent = document.getElementById("modal-column-3");
  const iframes = modalContent.getElementsByTagName("iframe");
  Array.from(iframes).forEach((iframe) => {
    iframe.src = ""; // Set src ke string kosong untuk menghentikan video
  });
}
...
async function kirimKeluhan(event) {
  ...
  try {
    ...
    // Menutup modal keluhan setelah menampilkan konfirmasi
    closeKeluhanModal();
